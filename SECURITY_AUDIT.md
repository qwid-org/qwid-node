...
>
> **DB-C2 failure semantics (note):** value transfer is enabled, but a reverting/erroring top-level contract call currently causes the whole block to be rejected (`EvaluateSmartContracts` returns false), rather than the Ethereum-style behavior of charging the fee and reverting only the call's state. That per-transaction failure model (fee charged on revert) depends on the deferred gas-economics work (Phase 3b). This is pre-existing behavior, now more consequential with value transfer live.
