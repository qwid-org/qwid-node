...
- **WH-C6** [CRITICAL] — A mutex-safe Call() wrapper was added but never adopted by any handler, so the response-mismatch race and single-channel DoS described in the finding are still live in practice. _(rpc/client/client.go:28-36 adds a mutex-protected Call() helper (tagged WH-C6), but grep shows 0 call sites use clientrpc.Call() in cmd/webui/handlers or cmd/website/handlers; all ~31+ call sites (e.g. cmd/webui/handlers/handlers.go:90,358,380,602... and cmd/website/handlers/wall)_
...
- **NP-M12** [MEDIUM] — A lock was added around the EncryptionOptData read, but it is the wrong mutex (voting.VotesEncryptionMutex instead of encryptionMutex used by the writers), so the data race with SetEncryptionData/ResetToDefaultEncryptionOptData is not actually resolved. _(services/nonceService/serviceNonce.go:43,50-51,59-60 (encryptionMutex guards writes to EncryptionOptData in ResetToDefaultEncryptionOptData/SetEncryptionData) vs. lines 137-144 (tagged '// NP-M12') which reads EncryptionOptData under a DIFFERENT lock, voting.VotesEncryptionMutex;)_
...
### PARTIAL / deferred — what remains (16)
...
- **CW-H3** [HIGH] — The wallet.go StoreJSON directory-creation call was hardened to 0700, but 3 of 4 real wallet-creation entry points (CLI, webui, Qt GUI) pre-create the directory as 0755, which wins because MkdirAll doesn't chmod existing dirs. _(wallet/wallet.go:541 now creates the wallet dir with os.MkdirAll(w.HomePath, 0700) inside StoreJSON(). However, since os.MkdirAll is a no-op on an already-existing directory, this fix is bypassed in practice: cmd/generateNewWallet/main.go:63, cmd/webui/handlers/handlers.go:254, a)_
...
- **CW-M1** [MEDIUM] — Empty-signature panic is fixed, but empty-message input to Verify() is still unguarded and can panic on the network-reachable verification path. _(wallet/wallet.go:938-942 checks 'if len(sig) < 1 { return false }' (tagged CW-M1) before any indexing. However, no equivalent check exists for 'msg': wallet.Verify() passes msg straight to oqs.Signature.Verify() (crypto/oqs/oqs.go:442-461), which does '&message[0]' at line 452 un)_
...
### FIXED (77) — verified remediated

- **Critical:** AC-C1, AC-C2, AC-C3, AC-C4, CW-C1, CW-C2, CW-C3, CW-C4, DB-C1, DB-C2, DB-C3, DB-C5, DB-C6, NP-C1, NP-C2, NP-C3, NP-C6, NP-C7, WH-C1, WH-C2, WH-C3, WH-C4, WH-C5
- **High:** AC-H1, AC-H2, AC-H3, AC-H5, AC-H6, AC-H7, CW-H1, CW-H4, CW-H5, CW-H6, DB-H1, DB-H2, DB-H3, DB-H7, DB-H8, NP-H11, NP-H13, NP-H14, NP-H4, NP-H7, NP-H8, WH-H1, WH-H2, WH-H4, WH-H6, WH-H7, WH-H8
- **Medium:** AC-M1, AC-M2, AC-M3, AC-M4, AC-M5, AC-M6, AC-M7, AC-M8, AC-M9, CW-M4, CW-M5, CW-M6, CW-M8, DB-M2, DB-M5, DB-M6, DB-M7, NP-M11, NP-M3, NP-M5, NP-M8, NP-M9, WH-M10, WH-M11, WH-M2, WH-M5, WH-M8
