...
> **Still open — deferred to Phase 3** (require the value-transfer/gas model): **DB-C2** (re-enable `CanTransfer`/`Transfer` for `msg.value`; note the currently-`nil` `CanTransfer` at `evm.go` CALLCODE must be wired then), **DB-C4** (gas pricing/refunds). Also deferred: wiring `PrepareAccessList` at tx-start (gas-cosmetic until Phase 3).
