...
> **Update 2026-07-12 (OB-120 NP DoS-resilience cluster):** **NP-H2** (per-topic concurrent inbound-connection cap `MaxInboundConnectionsPerTopic=64`, enforced in `Accept` after `admitPeer`/before the handshake, whitelist bypass), **NP-H6** (concurrent RPC-connection cap `MaxConcurrentRPCConnections=64` via a lock-free atomic semaphore `tryAcquireRPCSlot`/`releaseRPCSlot` in `ListenRPC`), and **NP-H10** (periodic tx re-broadcast is now delta-only — a goroutine-local `seen` hash-set means each round sends only newly-arrived txs, not the whole pool; on-arrival `BroadcastTxn` gossip and on-demand full-pool catch-up unchanged) are now **FIXED** — moved from the OPEN list above. See `docs/superpowers/specs/2026-07-12-np-dos-resilience-design.md`. Residual: per-IP RPC ban/rate-limiting for operator-overridden non-loopback binds remains a follow-up.
...
> **Update 2026-07-13 (OB-124 NP mediums cluster):** eight networking mediums are now **FIXED** — **NP-M1** (evict expired `bannedIP` entries so the map self-trims), **NP-M2** (non-blocking `ChanPeer` sends), **NP-M4** (reset the reconnect counter on a genuine received frame, not a fixed iteration cadence), **NP-M6** (exponential RPC dial backoff 1s→30s), **NP-M7** (drop the discarded per-call 1MB reply pre-alloc — gob sizes the slice), **NP-M10** (log dropped best-effort tx sends on a full channel), **NP-M13** (clamp attacker-controlled `gh` header-range requests to `NumberOfHashesInBucket`=20), **NP-M14** (cap peer IPs shared in `hi` to a bounded random subset via `sampleIPs`, `MaxPeersSharedInHi`=3). See `docs/superpowers/specs/2026-07-13-np-mediums-cluster-design.md`. **Accuracy note:** the original finding described `hi` as leaking the *full* connected-peer list, but `tcpip.GetIPsConnected()` already returns at most **one** random peer (`recieverTcpService.go`), so per-message exposure was already bounded upstream; `sampleIPs` is therefore defense-in-depth (it caps at 3 should `GetIPsConnected` ever return more) rather than a reduction of current behavior. NP-M14 residual (unchanged): repeated `hi` messages each reveal a random neighbor, so topology is still gradually inferable over time on this open-peering network — the accepted tradeoff to keep peer discovery working.

> **Update 2026-07-13 (OB-125 EVM/DB mediums cluster):** five EVM/DB mediums are now **FIXED** — **DB-M3** (`GVMLogger.ToString` now renders `ResultSCCall`, not `ResultTxCall`, for the SC-state/fault section), **DB-M4** (`GVMLogger` accumulation is bounded via `appendCapped`, 256 KB/field + a one-time truncation marker, capping `OutputLogs` memory/DB growth), **DB-M9** (`ForEachStorage`'s always-true `dirty` check simplified to the equivalent honest loop), **DB-M1** (the EIP-activation loop builds a filtered list of enabled EIPs instead of mutating `cfg.ExtraEips` while ranging — a dormant path, `ExtraEips` is always empty on this chain, so no live behavior change), and **`(CONSENSUS)` DB-M10** (`bigModExp.Run` now rejects operands longer than `MaxModExpLen`=1024 bytes with `ErrModExpOperandTooLarge`, bounding an OOM/DoS since gas is unenforced — DB-C4). See `docs/superpowers/specs/2026-07-13-db-mediums-cluster-design.md`. DB-M3/M4 are non-consensus (tracer output excluded from the tx-hash preimage); DB-M10's ceiling is above any realistic crypto (RSA-4096 = 512 B), so no legitimate contract is affected. The principled MODEXP/EVM bound remains real gas economics (DB-C4, deferred).

### FIXED (103) — verified remediated
...
- **Medium:** AC-M1, AC-M2, AC-M3, AC-M4, AC-M5, AC-M6, AC-M7, AC-M8, AC-M9, CW-M1, CW-M4, CW-M5, CW-M6, CW-M8, DB-M1, DB-M2, DB-M3, DB-M4, DB-M5, DB-M6, DB-M7, DB-M8, DB-M9, DB-M10, NP-M1, NP-M2, NP-M3, NP-M4, NP-M5, NP-M6, NP-M7, NP-M8, NP-M9, NP-M10, NP-M11, NP-M12, NP-M13, NP-M14, WH-M10, WH-M11, WH-M2, WH-M5, WH-M8
